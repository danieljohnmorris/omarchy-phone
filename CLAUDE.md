...
## postmarketOS components, where and why

Mainline Qualcomm hardware boots the modem/Wi-Fi remoteprocs but ships none of
the userspace Android ran to serve them. postmarketOS wrote that userspace; the
port is mostly collecting it one piece at a time.

| Piece | Role here |
|---|---|
| `linux-postmarketos-qcom-sdm845` 6.16.7 | Prebuilt kernel with fajita DTB. |
| `pd-mapper` + `tqftpserv` | Protection-domain services. Without pd-mapper the modem never appears and Wi-Fi does not come up either. |
| `rmtfs` | Remote filesystem the modem expects from the host. |
| `qrtr` (pinned revision) | Qualcomm IPC router; 81voltd and the modem talk over it. Pin is for reproducible builds. |
| ModemManager (patched, built here) + `qmicli` | Modem control: bearers, voice objects, messaging. Omarchy's build predates what we needed, hence local build. |
| `q6voiced` | Opens the hostless `VoiceMMode1` PCM (`hw:0,6`) on ModemManager call start/stop. Without it calls connect silent. |
| `81voltd` (`pmaports/temp/81voltd`, built here) | Server side of the QMI IMS Data service: when the modem asks for an IMS data connection, it drives ModemManager to connect a bearer on the `ims` APN. Upstream tested it working on Optus, not Telstra: carrier-dependent. |
| `alsa-ucm-conf` sdm845-mainline tree (pinned `1b8d290`) | UCM verbs `HiFi` and `Voice Call` for audio routing and profile flips. |
